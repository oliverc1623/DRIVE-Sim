...
def main():
    ...
    total_frames = 500  # Total number of frames for annealing
    print_interval = 1
    ...
    with open(
        f"../data/minigrid/Uniform-DQN-Unlock{sys.argv[1]}.csv", "w", newline=""
    ) as csvfile:
        ...
        for n_epi in range(600):
            ...
            # preprocess
            img1 = np.zeros((48,88,1)).astype(np.uint8)
            img2 = np.zeros((48,88,1)).astype(np.uint8)
            img3 = np.zeros((48,88,1)).astype(np.uint8)
            img4 = np.zeros((48,88,1)).astype(np.uint8)
