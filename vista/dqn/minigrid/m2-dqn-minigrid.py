...
class ReplayBuffer():
    ...
    def sample(self, n):
        ...
        return (torch.tensor(np.array(s_lst), dtype=torch.float32).permute(0,3,1,2).to(self.device))/ 255., torch.tensor(a_lst,dtype=torch.float32).to(self.device), \
               torch.tensor(np.array(r_lst), dtype=torch.float32).to(self.device), \
               (torch.tensor(np.array(s_prime_lst), dtype=torch.float32).permute(0,3,1,2).to(self.device))/ 255., \
