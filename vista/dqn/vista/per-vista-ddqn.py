...
class PrioritizedReplayBuffer:
    def __init__(
        ...
        self.state = torch.empty(
            buffer_size, 1, state_size_width, state_size_height, dtype=torch.int
        )
    ...
    def sample(self, batch_size):
        ...
        batch = (
            self.state[sample_idxs].to(self.device) / 255.0,
            self.action[sample_idxs].to(self.device),
            self.reward[sample_idxs].unsqueeze(1).to(self.device),
            self.next_state[sample_idxs].to(self.device) / 255.0,
...
def main():
    ...
    memory = PrioritizedReplayBuffer(
        128, 512, 1, buffer_limit, device, alpha=0.6, beta=0.4
    )
    ...
    with open(
        f"../data/vista/PER-DDQN-Vista{sys.argv[1]}.csv", "w", newline=""
    ) as csvfile:
