...
from helper import * 
from simple_vit_ac import SimpleViT
...
if __name__ == '__main__':
    ...
    while step_idx < max_train_steps:
        ...
        for _ in range(update_interval):
            # print(f"input shape: {s[0].permute(2,0,1).unsqueeze(0).to(device).shape}")
            mu0, sigma0 = model(s[0].permute(2,0,1).unsqueeze(0).to(device))
            mu1, sigma1 = model(s[1].permute(2,0,1).unsqueeze(0).to(device))
            mu2, sigma2 = model(s[2].permute(2,0,1).unsqueeze(0).to(device))
